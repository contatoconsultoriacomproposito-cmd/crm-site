...
export default function Navbar({
  ...
  // Função para abrir o ChatSDR globalmente
  const abrirChatIA = () => {
      ...
      <div className="bg-gray-900 text-white py-2 px-4 md:px-12 flex justify-between items-center text-[11px] md:text-xs font-medium">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-4">
            {contatos.instagram && (
              <a href={contatos.instagram} target="_blank" rel="noopener noreferrer" className="text-white hover:opacity-70 transition-opacity flex items-center gap-1">
                <FaInstagram size={14} />
                <span className="hidden sm:inline opacity-80">Instagram</span>
              </a>
            )}
            {contatos.facebook && (
              <a href={contatos.facebook} target="_blank" rel="noopener noreferrer" className="text-white hover:opacity-70 transition-opacity flex items-center gap-1">
                <FaFacebookF size={14} />
          ...
          <div className="hidden sm:flex items-center gap-2">
            ...
            <span className="opacity-80">{contatos.email_corporativo || "E-mail não informado"}</span>
          </div>
        </div>
      ...
      <div className="bg-white border-b border-gray-100 shadow-sm relative z-20">
        <div className="max-w-7xl mx-auto px-4 md:px-12 h-20 md:h-28 flex justify-between items-center">
          ...
          <div className="flex items-center">
            {logoFinal ? (
              <img src={logoFinal} alt={nome_exibicao} className="h-10 md:h-16 w-auto object-contain transition-transform hover:scale-105" />
            ) : (
              <h1 className="text-xl md:text-2xl font-black uppercase tracking-tighter" style={{ color: cor }}>
                {nome_exibicao}
              </h1>
            )}
          </div>
              ...
              <div className="absolute top-full left-0 w-64 bg-white shadow-2xl rounded-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 border border-gray-100 overflow-hidden">
                <div className="py-2 max-h-[350px] overflow-y-auto custom-scrollbar">
                  {produtosUnicos.map((nomeProduto) => (
                    <a 
                      key={nomeProduto}
                      href={`#${nomeProduto.replace(/\s+/g, '-').toLowerCase()}`} // REMOVIDO "seguro-" PARA SINCRONIZAR COM GRID
          ...
          <div className="flex items-center gap-4">
            {contatos.whatsapp_comercial && (
              <a href={`https://wa.me/${whatsappLimpo}`} target="_blank" rel="noopener noreferrer" className="hidden xl:flex items-center gap-3 group border-r border-gray-100 pr-6 mr-2">
            ...
            <button
              onClick={abrirChatIA} // GATILHO PARA O CHATSDR
            ...
            >
              <span className="relative h-2 w-2 hidden md:flex">
            ...
            <button onClick={() => setIsMobileMenuOpen(true)} className="lg:hidden p-2 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">
              <Menu size={28} />
            </button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[100] lg:hidden">
          ...
          <div className="absolute right-0 top-0 bottom-0 w-[300px] bg-white shadow-2xl p-8 flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <nav className="flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-4 max-h-[40vh] overflow-y-auto custom-scrollbar pr-2">
                {produtosUnicos.map(p => (
                  <a 
                    key={p} 
                    href={`#${p.replace(/\s+/g, '-').toLowerCase()}`} // REMOVIDO "seguro-" AQUI TAMBÉM
            ...
            <div className="mt-auto pt-8">
               <button
                onClick={() => { abrirChatIA(); setIsMobileMenuOpen(false); }}
